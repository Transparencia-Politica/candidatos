...
def test_parse_roll_call_extracts_orientation_and_votes():
    ...
    record, rows = ingest.parse_roll_call(votacao, votos, orientacoes)
    assert record["roll_call_id"] == "2383287-43"
    assert record["is_nominal"] is True
    assert record["gov_orientation"] == "Sim"
    assert record["opp_orientation"] == "Não"
    assert {"deputy_id": 74478, "vote_type": "Não"} in rows
    assert len(rows) == 2
...
def test_ingest_law_caches_nominal_rollcalls_and_skips_symbolic(conn):
    ...
    assert result["roll_calls"] == 1  # symbolic (empty votos) is skipped
    cached = db.get_deputy_votes(conn, camara_id=74478, law_ids=[lid])
    assert len(cached) == 1
    assert cached[0]["roll_call_id"] == "v-nom"
    assert cached[0]["vote_type"] == "Não"
